...
contract NFTStaking is Ownable, ERC721Holder, Pausable {
    ...
    function unpauseStaking() public onlyOwner whenPaused {
        ...
        // the contract can take staking from now on
        _unpause();
    }

    function setTokensClaimable(bool _enabled) public onlyOwner {
        //needs access control
        claimableTokens = _enabled;
    ...
    function _stake(address _user, uint256 _tokenId) internal {
        require(nft.ownerOf(_tokenId) == _user, "user is not owner of the token");
    ...
    function emergencyUnstake(uint256 _tokenId) public {
        require(tokenOwner[_tokenId] == msg.sender, "nft._unstake: Sender must have staked tokenID");
        _unstake(msg.sender, _tokenId);
    ...
    function updateReward(address _user) public {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            if (
                staker.tokenStakingCoolDown[ids[i]] < block.timestamp + stakingTime &&
                staker.tokenStakingCoolDown[ids[i]] > 0
            ) {
                uint256 stakedDays = ((block.timestamp - uint256(staker.tokenStakingCoolDown[ids[i]]))) / stakingTime;
                uint256 partialTime = ((block.timestamp - uint256(staker.tokenStakingCoolDown[ids[i]]))) % stakingTime;
    ...
    function claimReward(address _user) public {
        require(claimableTokens == true, "Tokens cannnot be claimed yet");
